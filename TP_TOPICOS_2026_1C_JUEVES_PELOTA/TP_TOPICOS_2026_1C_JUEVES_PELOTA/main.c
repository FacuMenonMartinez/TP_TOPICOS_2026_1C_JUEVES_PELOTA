...
/*
Apellido: Eguia Camejo, Agustin
DNI: 44870705
Usuario: AgustinEguia
Entrega: Sí

Apellido: Pueblas, Agustin
DNI: 41215377
Usuario: Aguuspueblas
Entrega: Sí

Apellido: Menon Martinez, Facundo
DNI: 41262481
Usuario: FacuMenonMartinez
Entrega: Sí
...
if(gbt_iniciar() !=0 ){
    ...
    ///Para la paleta mando paleta[config.paleta]
        ...
        config.velocidad = true?  (nivel = 5) : (nivel = 0);
    }
   ...
   //Inicializacion de variables
    ...
    uint16_t piezas_Fijadas = 0;    //Con 10 piezas hacer nivel++ //Controlar que nivel no desborde
...
if(crear_jugador)
   ...
   do
   {
       while(corriendo)
       {
           ...
           {
               printf("Game over\n");
               //Salir de la ejecucion del juego
               corriendo = 0;
           }
           ...
           puntaje = calcular_Puntaje(puntaje,filas_Eliminadas,descensos_Rapidos,nivel);
       ...
       reiniciar = mostrar_Pantalla_Game_Over(ranking);
       corriendo = 1;
        //Reinicializar todo lo que quedo del juego
        puntaje = 0;
   }
   while(reiniciar);
